...
- The connection payload does not expose a claim link. Route the merchant to the approved Replit Go Live/transfer surface so it can request a fresh claim URL on demand from OpenInt. Do not have generated app code call an OpenInt connector RPC directly to mint one. If the approved surface is not available for this flow, tell the merchant their transfer must be initiated through Replit's Go Live tooling and surface `pending_transfer` status from the connection if useful.
