...
export function AppSidebar() {
  ...
  const combatLevel = state ? getCombatLevel(state as GameState) : null;
  const smithingLevel = state ? calculateLevel(state.smithingXp) : null;
      ...
      <div className="px-3 py-4 border-b border-border">
        ...
        {state && (
          <div className="mt-2 flex items-center gap-2 text-xs text-muted-foreground">
            ...
            {state.bones > 0 && <span>🦴 {formatNumber(state.bones)}</span>}
          </div>
        )}
      </div>
      ...
      <nav className="flex-1 py-1">
        ...
        {GATHERING_SKILLS.map(skill => (
          <NavItem
            ...
            icon={skill.icon}
            color={skill.color}
            level={state ? calculateLevel(state[skill.xpKey]) : null}
        ...
        <NavItem title="Smithing" url="/smithing" icon={Shield}
          level={smithingLevel} isActive={location === "/smithing"} color="text-slate-400" />
      </nav>
